...
var Utils = /*#__PURE__*/function () {
  ...
  Utils.sleep = function sleep(ms) {
    var promise_sleep = util.promisify(setTimeout);
...
var I18nLoader = /*#__PURE__*/function () {
  ...
  _proto.load = function load(custom) {
    try {
      ...
      _this2.watcher[lang] = chokidar.watch(file, {
        ignored: /(^|[/\\])\../,
        // ignore dotfiles
        persistent: true
      }); //Add change watcher
...
var TokenGenerator = /*#__PURE__*/function () {
  ...
  _proto.refresh = function refresh(token) {
    var payload = jsonwebtoken.verify(token, this.privateKey, this.options);
    delete payload.sub;
    delete payload.iss;
    delete payload.aud;
    delete payload.iat;
...
var Server = /*#__PURE__*/function () {
  ...
  _proto.config = function config(_config) {
    if (_config && _config.helmet) {
      //Security
      this.app.use(helmet(_config && lodash.isObject(_config.helmet) && _config.helmet));
    }
    ...
    if (_config && _config.cors) {
      ...
      this.app.options("*", cors(_config && lodash.isObject(_config.cors) && _config.cors));
      this.app.use(cors(_config && lodash.isObject(_config.cors) && _config.cors));
    }
  ...
  _proto.loadRoutes = function loadRoutes(app, routes) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(routes), _step; !(_step = _iterator()).done;) {
      ...
      var router = route.configure(); //perform autoconfig for base api crud operations
...
var ClusterServer = /*#__PURE__*/function (_EventEmitter) {
    ...
    try {
      ...
      var _temp4 = function () {
        if (cluster.isPrimary) {
          ...
          var messages = new ClusterMessages();
          ...
          var cpuCount = os.cpus().length; //Create a worker for each CPU
  ...
  _proto.initUnclustered = function initUnclustered() {
    try {
      ...
      var server = http.Server(_this7.server.app);
        ...
        function _temp8() {
          function _temp6() {
            ...
            if (process.env.SSL && process.env.SSL == "true") {
              ...
              var serverSsl = https.createServer(options, _this7.server.app);
...
var EventHandler = /*#__PURE__*/function (_EventEmitter) {
  ...
  function EventHandler(app) {
    ...
    _this = _EventEmitter.call(this) || this;
    _this.messages = new ClusterMessages();
...
var _configure = log4js.configure,
...
var Logger = /*#__PURE__*/function () {
  ...
  Logger.configure = function configure() {
    try {
      var readfile = util.promisify(fs.readFile);
      return Promise.resolve(readfile(path.resolve(process.cwd(), "./log4js.json"), "utf8")).then(function (json) {
  ...
  _proto.validate = function validate(request, username, password) {
    try {
      ...
      return Promise.resolve(_this5.userDao.findByUsername(username)).then(function (user) {
        return user && user.username === username && user.password === Utils.encrypt(password) ? _this5.tokenGenerator.sign(lodash.omit(user, ['password'])) : false;
      });
    } catch (e) {
      return Promise.reject(e);
    }
...
var KnexFilterParser = /*#__PURE__*/function () {
  ...
  KnexFilterParser.parseQueryString = function parseQueryString(builder, string, tableName) {
    ...
    var parser = new FQLParser(options);
  ...
  _proto.loadFilteredData = function loadFilteredData(filters, start, limit) {
    try {
      ...
      return Promise.resolve(KnexConnector$1.connection.from(_this2.tableName).where(function (builder) {
        return KnexFilterParser.parseFilters(builder, lodash.omit(filters, ["sort", "start", "limit"]), _this2.tableName);
  ...
  _proto.countFilteredData = function countFilteredData(filters) {
    try {
      ...
      return Promise.resolve(KnexConnector$1.connection.from(_this4.tableName).where(function (builder) {
        return KnexFilterParser.parseFilters(builder, lodash.omit(filters, ["sort", "start", "limit"]), _this4.tableName);
      }).count("id", {
        as: "total"
      })).then(function (data) {
...
var Runtime = function Runtime(extra) {
  try {
    var _exit;

    var _iterator, _step, param, _iterator2, _step2, _param;

    var cfg = yargs(hideBin(process.argv)).usage("Como usar: \n            node $0 [options] \n            \n            ** Si no se especifican par\xE1metros el servidor arrancar\xE1 normalmente. **").alias("g", "generateKeys").describe("g", "Genera unas claves para la aplicación").alias("c", "encrypt").describe("c", "Codifica el String proporcionado en base a la contraseña de .env").nargs("c", 1).help("h").alias("h", "help");
    var demand = [];

    if (extra) {
      for (_iterator = _createForOfIteratorHelperLoose(extra); !(_step = _iterator()).done;) {
        param = _step.value;
    ...
    var argv = cfg.argv; //Parámetro para no arrancar el servidor y generar las claves JWT
    ...
    return Promise.resolve(function () {
      if (extra) {
        _iterator2 = _createForOfIteratorHelperLoose(extra);
        return _for(function () {
          return !_exit && !(_step2 = _iterator2()).done;
        }, void 0, function () {
          _param = _step2.value;
          return function () {
            if (argv[_param.key]) {
              return Promise.resolve(_param.fn(argv)).then(function () {
                var _process$exit = process.exit(1);
...
var App = /*#__PURE__*/function () {
  ...
  _proto.runtime = function runtime(extra) {
