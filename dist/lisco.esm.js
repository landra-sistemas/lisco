...
class I18nLoader {
    ...
    async load(custom) {
        ...
        let file = path$1.resolve(process.cwd(), "i18n/lang_" + lang + ".json");
        try {
...
class ClusterServer extends EventEmitter {
    ...
    async initUnclustered() {
        ...
        if (process.env.SSL && process.env.SSL == "true") {
            ...
            var key = fs.readFileSync(path$1.resolve(process.cwd(), process.env.SSL_KEY || "key.pem"));
            var cert = fs.readFileSync(path$1.resolve(process.cwd(), process.env.SSL_CERT || "cert.pem"));
...
class Logger {
    static async configure() {
        ...
        const json = await readfile(path$1.resolve(process.cwd(), "./log4js.json"), "utf8");
