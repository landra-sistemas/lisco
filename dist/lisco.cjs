 ...
 */
class ClusterServer extends require$$0$2.EventEmitter {
    constructor(app) {
        super();
    ...
    async initClustered() {
        ...
        if (cluster__default["default"].isPrimary) {
            ...
            const cpuCount = require$$2__default["default"].cpus().length;
    ...
    async initUnclustered() {
        ...
        if (process.env.SSL && process.env.SSL == "true") {
            ...
            var key = fs__default["default"].readFileSync(require$$0__default["default"].resolve(process.cwd(), process.env.SSL_KEY || "key.pem"));
            var cert = fs__default["default"].readFileSync(require$$0__default["default"].resolve(process.cwd(), process.env.SSL_CERT || "cert.pem"));
 ...
 */
class EventHandler extends require$$0$2.EventEmitter {
    constructor(app) {
        super();
...
class Logger {
    static async configure() {
        ...
        const json = await readfile(require$$0__default["default"].resolve(process.cwd(), "./log4js.json"), "utf8");
...
class BaseKnexDao {
    ...
    async loadFilteredData(filters, start, limit) {
        ...
        return KnexConnector$1.connection
            .from(this.tableName)
            .where((builder) => KnexFilterParser.parseFilters(builder, lodash__default["default"].omit(filters, ["sort", "start", "limit"]), this.tableName))
            .orderByRaw(sorts)
            .limit(limit)
            .offset(start);
    }
...
var exports$1 = optimist.exports = Argv;
